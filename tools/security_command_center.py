...
from grid.security.security_monitoring import SecurityMonitor, ThreatLevel
...
class SecurityCommandCenter:
    ...
    def profile_threats(self) -> ThreatProfile:
        ...
        weighted_score = sum(
            profile.severity_distribution.get(sev, 0) * weight
    ...
    def _generate_recommendations(self, profile: ThreatProfile) -> list[str]:
        ...
        if profile.detection_accuracy < 0.95:
            recommendations.append(
                f"Review detection thresholds - accuracy at {profile.detection_accuracy:.1%}"
            )
        ...
        if critical_count > 0:
            recommendations.append(
                f"URGENT: {critical_count} critical events detected - immediate review required"
            )
        
        # Check attack patterns
        if len(profile.top_attack_sources) > 5:
            recommendations.append(
                "Multiple attack sources detected - consider IP blocking or rate limiting"
            )
        
        # Check risk score
        if profile.risk_score > 50:
            recommendations.append(
                f"Risk score elevated ({profile.risk_score:.1f}/100) - review security posture"
            )
        
        # Check for missing detections
        if profile.threat_categories.get("parasite.detected", 0) == 0:
            recommendations.append(
                "No parasite detections recorded - verify Parasite Guard is enabled"
            )
