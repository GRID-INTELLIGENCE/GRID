...
SENSITIVE_MARKERS = ("KEY", "TOKEN", "SECRET", "PASSWORD", "PASS", "BEARER", "AUTH", "CREDENTIAL", "API")
...
class EnvironmentAuditor:
    ...
    def __init__(self):
        self.audit_results = {
            ...
            "memory_profile": {"total_objects": 0, "top_object_types": {}, "garbage": 0},
            "system_resources": {"system": {}, "process": {}},
            "recommendations": [],
        }
    ...
    def _analyze_environment(self):
        ...
        real_conflicts = {
            ...
            if len(v) > 1 and not k.startswith("_") and not any(stdlib_path in p for p in v)
        }
            ...
            if path_info["exists"] and path_info["is_directory"]:
                try:
                    ...
                    path_info["contains_python_files"] = any(
                        f.endswith(".py") or f.endswith(".pyd") or f.endswith(".so") for f in files
                    )
    ...
    def _generate_recommendations(self):
        ...
        if (
            self.audit_results["environment_analysis"]["security_checks"]["parasite_guard"]["status"]
            == "not_configured"
        ):
...
def main():
    ...
    try:
        ...
        print(
            f"Security issues found: {len([r for r in results['recommendations'] if r['severity'] in ['high', 'critical']])}"
        )
        print(f"Recommendations: {len(results['recommendations'])}")
        print(
            f"Environment variables: {results['environment_analysis']['environment_vars_count']} total, "
