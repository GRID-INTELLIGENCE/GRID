...
class RuntimeBugScanner(ast.NodeVisitor):
    ...
    def visit(self, node: ast.AST):
        ...
        self.current_line = node.lineno if hasattr(node, 'lineno') else self.current_line
...
def scan_file_with_regex(file_path: Path) -> List[BugReport]:
    ...
    pattern_nan = re.compile(r'float\s*\(\s*(?:row|df|series|dataframe)\.(?:get|__getitem__|iloc|loc)\s*\([^)]+\)', re.IGNORECASE)
    ...
    for i, line in enumerate(lines, 1):
        ...
        if match and 'is not None' not in line and 'is None' not in line:
            ...
            if var_name.lower() not in ['true', 'false', 'self', 'cls', 'none', 'len', 'bool']:
                ...
                if (f'Optional[{var_name}' in context or
                    f'{var_name}: Optional' in context or
                    f'{var_name} = None' in context or
                    f'{var_name} | None' in context):
    ...
    pattern_conversion = re.compile(r'(int|float|str)\s*\((?:request|input|row|df|data|value|arg|param|config|env)', re.IGNORECASE)
    for i, line in enumerate(lines, 1):
    ...
    for i, line in enumerate(lines, 1):
        ...
        if match:
            ...
            if (f'{denominator} != 0' not in context and
                f'{denominator} > 0' not in context and
                f'{denominator} == 0' not in context and
                f'if {denominator}' not in context.lower()):
                # Only flag if denominator could plausibly be zero (user input, calculation result)
                if any(keyword in context.lower() for keyword in ['input', 'request', 'calculate', 'compute', 'get', 'fetch', 'parse']):
                    bugs.append(BugReport(
                        file_path=str(file_path),
                        line_number=i,
                        pattern_type="DIVISION_BY_ZERO",
...
def scan_directory(directory: Path, include_patterns: List[str] = None) -> List[BugReport]:
    """Scan all Python files in a directory."""
    all_bugs: List[BugReport] = []

    # Exclude patterns (virtual envs, test files with mocks, etc.)
    exclude_patterns = [
        '.venv', '__pycache__', '.pytest_cache', 'site-packages',
...
def generate_report(bugs: List[BugReport], output_dir: Path):
    ...
    inventory = {
        ...
        "total_bugs": len(bugs),
        "by_pattern": {
            pattern: len(bug_list) for pattern, bug_list in by_pattern.items()
        },
