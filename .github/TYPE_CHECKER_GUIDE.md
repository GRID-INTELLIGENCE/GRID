...
## Fix by category (workspace-wide)

For breakdown by diagnostic code and top files, see [../docs/ISSUES_BREAKDOWN.md](../docs/ISSUES_BREAKDOWN.md). Common categories:

- **reportMissingImports / reportUndefinedVariable:** Install deps or use conditional imports (pattern 4 above).
- **reportOptionalMemberAccess:** Use explicit `is not None` checks (patterns 1 and 3).
- **reportArgumentType / reportReturnType / reportAssignmentType:** Use `float | None`, correct handler types (e.g. `Exception` for exception handlers); see [../docs/TYPECHECK_FIXES_APP_CORE.md](../docs/TYPECHECK_FIXES_APP_CORE.md) for app core examples.
- **reportAttributeAccessIssue / reportCallIssue:** Align with current API (config attributes, parameter names) or add type narrowing.
