...
class GridIntegrationTests:
    ...
    def test_skills_pipeline(self):
        ...
        if success:
            try:
                data = json.loads(output)
                self.print_test("context.refine", data.get('status') == 'success', f"Output: {data.get('output', '')[:40]}")
            except:
        ...
        args = json.dumps({"text": "Test framework", "target_schema": "resonance", "output_format": "json", "use_llm": False})
        cmd = f'{self.venv} -m grid skills run transform.schema_map --args-json \'{args}\''
        success, output, _ = self.run_command(cmd)
        if success:
            try:
                data = json.loads(output)
                self.print_test("transform.schema_map", data.get('status') == 'success', f"Schema: {data.get('target_schema')}")
            except:
        ...
        args = json.dumps({"text": "This is a long text that needs to be compressed into a shorter version.", "max_chars": 50, "use_llm": False})
        ...
        if success:
            try:
                ...
                self.print_test("compress.articulate", data.get('status') == 'success' and chars <= 50, f"Compressed to {chars} chars")
            except:
    ...
    def test_rag_system(self):
        ...
        self.print_test("Ollama availability", ollama_running, "Ollama is " + ("running" if ollama_running else "not running"))
    ...
    def test_integration(self):
        ...
        if success1:
            try:
                ...
                args = json.dumps({"text": refined, "target_schema": "resonance", "output_format": "json", "use_llm": False})
                cmd = f'{self.venv} -m grid skills run transform.schema_map --args-json \'{args}\''
