...
- **`D:\build\grid`** - Cognitive framework with RAG & observability (Python, FastAPI) - PRIMARY
- **`D:\build\Coinbase`** - Crypto-focused application with GRID integration
- **`D:\build\analysis_outputs`** - Shared analysis artifacts repository

## Critical Architectural Patterns

### 1. GRID Framework Structure (`D:\build\grid`)
...
```bash
# Python tests (pytest)
cd D:\build\grid && python -m pytest tests/ -v
...
### Python Path
Ensure `D:\build\grid\src` is in PYTHONPATH when running tests or scripts.
