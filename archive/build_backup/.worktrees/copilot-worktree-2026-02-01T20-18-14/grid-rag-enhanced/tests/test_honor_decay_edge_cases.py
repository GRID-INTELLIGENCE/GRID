...
# Add Arena path to sys.path (optional; skip entire module if the_chase not present)
arena_path = Path(__file__).parent.parent / "archive" / "misc" / "Arena" / "the_chase" / "python" / "src"
if arena_path.exists() and str(arena_path) not in sys.path:
...
try:
    ...
    pytest.skip("the_chase not available (archive/misc/Arena not present)", allow_module_level=True)
