...
async def login(
    ...
    **Production Mode:**
    ...
    # For now, accept any credentials in development mode
    if not settings.is_development:
        ...
        logger.info("Production login attempted for user: %s", request.username)
...
async def logout(
    ...
    **Note:** JWT tokens are stateless, so true invalidation requires
    a token blacklist or revocation list (not yet implemented).

    For now, this endpoint serves as a logout confirmation.
    ...
    # TODO: In production, add token to revocation list
    # - Store token JTI in Redis/database
