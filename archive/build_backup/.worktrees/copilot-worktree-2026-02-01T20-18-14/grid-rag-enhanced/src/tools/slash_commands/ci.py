...
class CICommand(PipelineCommand):
    ...
    async def _run_unit_tests(self, context: CommandContext) -> dict[str, Any]:
        ...
        try:
            ...
            passed_count = output.count("passed") if result.returncode == 0 else 0
            failed_count = output.count("failed") if result.returncode != 0 else 0
    ...
    async def _run_rag_contracts(self, context: CommandContext) -> dict[str, Any]:
        ...
        try:
            ...
            # Parse test results
            output = result.stdout
            passed = output.count("passed") if result.returncode == 0 else 0
    ...
    async def _run_quality_check(self, context: CommandContext) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                result = subprocess.run(
                    ["grep", "-r", "-n", "print(", "src/", "--include=*.py"], capture_output=True, text=True, timeout=30
                )
                if result.stdout.strip():
            ...
            try:
                ...
                if result.stdout.strip():
                    long_line_files = len(result.stdout.strip().split("\n"))
    ...
    async def _run_security_scan(self, context: CommandContext) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if result.stdout.strip():
                    eval_count = len(result.stdout.strip().split("\n"))
