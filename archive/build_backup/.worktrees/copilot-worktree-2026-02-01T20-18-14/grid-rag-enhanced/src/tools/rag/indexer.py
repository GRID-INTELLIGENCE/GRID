...
def index_repository(
    ...
    if agent_ignore:
        if isinstance(exclude_dirs, set):
            exclude_dirs.update(agent_ignore)
        else:
            exclude_dirs.extend(agent_ignore)
...
def update_index(
    ...
    if is_databricks_store:
        ...
        dbx_tracker = DatabricksManifestTracker(
            ...
            table=config.databricks_manifest_table,
    ...
    if dbx_tracker is not None:
        ...
        for p in current_files:
            ...
            current_rel_paths.add(Path(rel_path).as_posix())
        ...
        for file_path in changed_files:
            ...
            chunker = SemanticChunker(
                min_chunk_size=50, max_chunk_size=config.chunk_size, chunk_overlap=config.chunk_overlap
            )
            semantic_chunks = chunker.chunk_file(content, rel_path_str)
        ...
        for i, sc in enumerate(semantic_chunks):
            try:
                ...
                full_metadata = {
                    ...
                    "file_size": len(content),
                    "file_hash": file_hash,
                    "indexed_at": tracker.state.last_updated or "",
                    **metadata_base,
                }
