...
async def read_resource(uri: AnyUrl) -> ReadResourceResult:
    ...
    try:
        ...
        if uri_str == "mastermind://project-info":
            ...
            return ReadResourceResult(contents=[TextContent(type="text", text=json.dumps(info, indent=2))])  # type: ignore
        elif uri_str == "mastermind://code-analysis":
            analysis = await analyze_codebase()
            return ReadResourceResult(contents=[TextContent(type="text", text=json.dumps(analysis, indent=2))])  # type: ignore
        elif uri_str == "mastermind://dependencies":
            deps = await analyze_dependencies()
            return ReadResourceResult(contents=[TextContent(type="text", text=json.dumps(deps, indent=2))])  # type: ignore
        elif uri_str == "mastermind://test-coverage":
            coverage = await analyze_test_coverage()
            return ReadResourceResult(contents=[TextContent(type="text", text=json.dumps(coverage, indent=2))])  # type: ignore
        else:
            raise ValueError(f"Unknown resource: {uri_str}")
    except Exception as e:
        logger.error(f"Error reading resource {uri}: {e}")
        return ReadResourceResult(contents=[TextContent(type="text", text=f"Error: {str(e)}")])  # type: ignore
