...
def find_python_files(root: Path, exclude_patterns: List[str], max_files: int = 10000) -> Dict[str, List[Path]]:
    ...
    return {
        'included': included,
        'excluded': excluded,
        'excluded_by_pattern': dict(excluded_by_pattern)
    }
...
def main():
    ...
    if excluded_by_pattern:
        ...
        for pattern, files in sorted(excluded_by_pattern.items(), key=lambda x: len(x[1]), reverse=True):
    ...
    if included_analysis and included_analysis.get('max_size', 0) > 500 * 1024:  # 500KB
        print(f"  WARNING: Large files detected. Consider excluding:")
