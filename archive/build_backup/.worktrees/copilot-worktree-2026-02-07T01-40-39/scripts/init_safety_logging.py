...
class SafetyEvent:
    ...
    denylist_reason: Optional[str] = None
    ...
    def log_event(self, event: SafetyEvent):
        ...
        if event.event_type == EventType.SERVER_DENIED.value:
            self.metrics.record_denial(
                event.server_name or "unknown",
                event.denylist_reason or "unknown",
                event.safety_score or 0.0
            )
...
def init_safety_logging(target_dir: str):
    ...
    event = create_safety_event(
        ...
        context={
            'action': 'safety_logging_initialized',
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description="Initialize AI Safety logging for Server Denylist System"
    )
    parser.add_argument(
        '--target',
        required=True,
        help='Target directory for safety logging'
    )
