...
class SafetyAwareServerManager(ServerDenylistManager):
    ...
    def __init__(self, config_path: str, safety_log_dir: str = None):
    ...
    def is_denied(self, server_name: str) -> tuple[bool, str]:
        ...
        if self.safety_logger:
            ...
            severity = self._determine_severity(reason) if is_denied else Severity.INFO
    ...
    def apply_to_mcp_config(self, mcp_config_path: str, output_path: str = None):
        ...
        if self.safety_logger:
            ...
            end_event = create_safety_event(
                ...
                metadata={
                    'denied_servers': [name for name, _ in denied_servers]
                }
            )
            ...
            if is_denied and reason in ['security-concern', 'startup-failure']:
                ...
                if self.safety_logger:
                    event = create_safety_event(
                        ...
                        context={
                            'violation_type': reason,
                            'requires_immediate_action': safety_score < 0.3
                        }
                    )
                    self.safety_logger.log_event(event)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="AI Safety-Aware Server Denylist Manager"
    )
