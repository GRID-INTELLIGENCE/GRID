...
def compare_baselines(current: Dict, previous: Dict) -> Dict:
    ...
    if previous.get('included_count', 0) > 0:
        comparison['file_count_change_percent'] = (
            comparison['file_count_change'] / previous['included_count'] * 100
        )
    
    if previous.get('total_size_bytes', 0) > 0:
        comparison['size_change_percent'] = (
            comparison['size_change_bytes'] / previous['total_size_bytes'] * 100
        )
