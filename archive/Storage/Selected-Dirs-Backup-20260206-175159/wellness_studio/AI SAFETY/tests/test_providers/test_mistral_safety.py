"""
Mistral Safety Engine Tests
Tests for Safety Charter, Multilingual Safety, and Open Model Safety
"""
import unittest
import json
...
class TestMistralSafetyEngine(unittest.TestCase):
    """Test Mistral AI's safety frameworks."""

    def setUp(self):
        """Load Mistral safety configuration."""
        self.config_dir = Path(__file__).parent.parent.parent / "PROVIDERS" / "MISTRAL"
        self.schema = self._load_json("MISTRAL_AI_SAFETY_SCHEMA.json")
    ...
    def test_schema_structure(self):
        """Test that Mistral schema has required fields."""
        ...
        self.assertEqual(self.schema["provider"], "Mistral AI")

    def test_safety_charter(self):
        """Test Safety Charter is defined."""
        if not self.has_schema:
            self.skipTest("Schema file not found")
        frameworks = self.schema.get("safety_frameworks", {})
        self.assertIn("safety_charter", frameworks)

    def test_open_model_safety(self):
        """Test Open Model Safety Protocol is defined."""
        if not self.has_schema:
            self.skipTest("Schema file not found")
        frameworks = self.schema.get("safety_frameworks", {})
        self.assertIn("open_model_safety_protocol", frameworks)

    def test_multilingual_safety(self):
        """Test multilingual safety is defined."""
        if not self.has_schema:
            self.skipTest("Schema file not found")
        research_areas = self.schema.get("research_areas", [])
        multilingual = [r for r in research_areas if "multilingual" in r.get("name", "").lower()]
        self.assertGreater(len(multilingual), 0)
    ...
    def test_safety_filters(self):
        """Test safety filters are defined."""
        if not self.has_schema:
            self.skipTest("Schema file not found")
        monitoring = self.schema.get("monitoring_parameters", {})
        self.assertIn("safety_filters", monitoring)
