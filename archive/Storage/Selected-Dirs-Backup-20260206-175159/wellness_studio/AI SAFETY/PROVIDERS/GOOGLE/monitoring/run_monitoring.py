#!/usr/bin/env python3
"""
Scheduled monitoring runner for Google DeepMind & Gemini safety research sources.

- Fetches configured sources (DeepMind Safety, AI Principles, Frontier Framework)
- Applies Google-specific keyword filters
- Stores snapshots and generates diffs
...
DEFAULT_CONFIG_PATH = "AI SAFETY/PROVIDERS/GOOGLE/monitoring/monitoring_config.json"
...
def repo_root() -> Path:
    # .../wellness_studio/AI SAFETY/PROVIDERS/GOOGLE/monitoring/run_monitoring.py
    # parents[4] -> .../wellness_studio
...
def fetch_url(url: str, timeout: int) -> tuple[int, str, StrDict]:
    req = Request(url, headers={"User-Agent": "Google-Safety-Monitor/1.0"})
...
def run_monitoring(config_path: Path, timeout: int) -> int:
    ...
    log_path = resolve_output_path(
        ...
        get_str_from_dict(
            outputs,
            "log_path",
            "AI SAFETY/PROVIDERS/GOOGLE/monitoring/logs/monitoring.log",
        ),
    )
    status_path = resolve_output_path(
        ...
        get_str_from_dict(
            outputs,
            "status_path",
            "AI SAFETY/PROVIDERS/GOOGLE/monitoring/status/last_run.json",
        ),
    )
    diff_dir = resolve_output_path(
        root,
        get_str_from_dict(
            outputs, "diff_path", "AI SAFETY/PROVIDERS/GOOGLE/monitoring/diffs"
        ),
    )
    reports_dir = resolve_output_path(
        root,
        get_str_from_dict(
            outputs, "reports_path", "AI SAFETY/PROVIDERS/GOOGLE/monitoring/reports"
        ),
    )
    snapshots_dir = resolve_output_path(
        root,
        get_str_from_dict(
            outputs, "snapshots_path", "AI SAFETY/PROVIDERS/GOOGLE/monitoring/snapshots"
        ),
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run scheduled monitoring for Google AI safety sources."
    )
    parser.add_argument(
        "--config",
