...
class RateLimiter:
    ...
    def _update_state(self, state: Dict, current_time: float, flow_bonus: float = 1.0):
        ...
        effective_bonus = flow_bonus if flow_bonus != 1.0 else (
            self.config.stamina_flow_bonus if state['consecutive_safe'] >= 5 else 1.0
        )
...
class TemporalSynchronizationEngine:
    ...
    def __init__(self, config: TemporalSafetyConfig, session_start: Optional[float] = None):
        self.config = config
        ...
        self.session_start: float = session_start or time.time()
    ...
    def should_allow_response(self, current_time: Optional[float] = None) -> tuple[bool, Optional[str]]:
        ...
        if len(self.burst_responses) >= self.config.max_burst_responses:
            ...
            if time_since_burst_start < self.config.burst_window:
                return False, f"Burst limit exceeded ({len(self.burst_responses)} responses in {time_since_burst_start:.1f}s)"

        # Check session duration
        session_duration = now - self.session_start
        if session_duration > self.config.max_session_length:
            return False, f"Session duration limit exceeded ({session_duration:.0f}s > {self.config.max_session_length}s)"

        return True, None

    def record_response(self, response_time: Optional[float] = None):
        """Record a response for temporal tracking"""
        now = response_time or time.time()
        self.last_response_time = now
        self.burst_responses.append(now)

    def get_temporal_pattern(self, current_time: Optional[float] = None) -> TemporalPattern:
...
class HookDetectionEngine:
    ...
    def _identify_cognitive_impacts(self) -> List[str]:
        ...
        short_interactions = sum(1 for r in self.interaction_history
                               if r.user_input_length < 10 and r.ai_response_length < 50)
...
class UserWellbeingTracker:
    ...
    def __init__(self, config: TemporalSafetyConfig, user_age: Optional[int] = None):
        self.config = config
    ...
    def update_metrics(self, interaction: InteractionRecord) -> UserWellbeingMetrics:
        ...
        self.current_metrics.session_duration = time.time() - self.current_metrics.last_wellbeing_check

        # Update metrics periodically
        if (self.current_metrics.total_interactions % self.config.wellbeing_check_frequency == 0 or
            time.time() - self.current_metrics.last_wellbeing_check > 300):  # every 5 minutes
            ...
            self.current_metrics.last_wellbeing_check = time.time()
    ...
    def _calculate_behavioral_health(self):
        ...
        combined_risk = (self.current_metrics.behavioral_loop_risk +
                        self.current_metrics.interaction_density_score) / 2
    ...
    def _calculate_developmental_safety(self):
        ...
        max_risk = max(self.current_metrics.attention_span_risk,
    ...
    def _detect_manipulation_patterns(self, user_input: str) -> Dict[str, Any]:
        ...
        if not hasattr(self, '_manipulation_patterns'):
            self._manipulation_patterns = [
                ...
                (re.compile(r"(?i)(trust\s+me\s+(?:only|more\s+than\s+(?:anyone|your\s+parents?)))"), "trust_manipulation"),
    ...
    def _detect_grooming_escalation(self) -> Dict[str, Any]:
        ...
        escalation_detected = (
            intimacy_score["escalation"] > 0.3 or
            secrecy_pattern["escalation"] > 0.3 or
            boundary_pattern["escalation"] > 0.3
        )
    ...
    def _detect_time_pattern(self, timestamps: List[float]) -> Dict[str, Any]:
        ...
        if len(hours) >= 3:
            ...
            return {
                ...
                "pattern_type": "late_night" if unusual_ratio > 0.7 else "regular_schedule" if regularity_score <= 2 else None
            }
    ...
    def _analyze_secrecy_escalation(self, interactions: List[InteractionRecord]) -> Dict[str, Any]:
        ...
        secrecy_counts = []
        for interaction in interactions:
            count = sum(1 for marker in ["secrecy_request", "isolation_attempt"]
    ...
    def _analyze_boundary_escalation(self, interactions: List[InteractionRecord]) -> Dict[str, Any]:
        ...
        boundary_counts = []
        for interaction in interactions:
            count = sum(1 for marker in ["boundary_testing", "boundary_crossing"]
                       if interaction.cognitive_markers.get(marker, False))
...
class AIWorkflowSafetyEngine:
    ...
    def __init__(self, user_id: str, config: TemporalSafetyConfig, user_age: Optional[int] = None, session_start: Optional[float] = None):
        ...
        logger.info("AI Workflow Safety Engine initialized",
    ...
    async def evaluate_interaction(self,
                                 ...
                                 sensitive_detections: int = 0) -> Dict[str, Any]:
        """
        ...
        # Get consistency score from tracker
        ...
        rate_limit_result = self.rate_limiter.check_rate_limit(
            self.user_id, user_input, now, flow_bonus=flow_bonus
        )
        stamina_allowed = rate_limit_result['allowed']
        stamina_reason = rate_limit_result.get(
            'stamina_reason',
            "Stamina exhausted" if not stamina_allowed else "OK"
        )
        remaining_stamina = rate_limit_result['stamina_remaining']

        # 0.5 Manipulative Pattern Pre-check (for cognitive markers)
        ...
        # Temporal safety check (Traditional)
        ...
        # 2. Rule 2: Deterministic Heat
        # Heat_Generated = (SensitiveDetections * 10) + (InteractionDensity * 5)
        # We also decay heat over time (cap elapsed to avoid negative = heat explosion when clocks differ)
        last_check = metrics.last_wellbeing_check or now
        elapsed = max(0.0, now - last_check)
        self.current_heat = max(0.0, self.current_heat - (elapsed * 2.0))  # Decay 2 heat per second

        # Cap density contribution so old/skewed timestamps don't blow up heat
        ...
        safety_allowed = (
            stamina_allowed and
            temporal_allowed and
            hook_analysis.risk_level != HookRisk.CRITICAL and
            self.current_heat <= self.config.heat_threshold and
            dev_safety["is_safe"]
        )

        # blocked_reason when safety_allowed is False (for middleware and tests)
        blocked_reason = None
        if not safety_allowed:
            if not stamina_allowed:
                blocked_reason = "STAMINA_EXHAUSTED"
            elif not temporal_allowed:
                blocked_reason = "TEMPORAL_VIOLATION"
            elif hook_analysis.risk_level == HookRisk.CRITICAL:
                ...
                blocked_reason = dev_safety.get("reasons", ["SAFETY_VIOLATION"])[0]
        ...
        # Record the response if allowed
        if safety_allowed:
        ...
        if not safety_allowed or hook_analysis.risk_level in [HookRisk.HIGH, HookRisk.CRITICAL]:
            logger.warning("AI workflow safety event",
                         safety_allowed=safety_allowed,
                         hook_risk=hook_analysis.risk_level.value,
                         temporal_reason=temporal_reason or stamina_reason,
                         heat=self.current_heat)

        return assessment

    def _make_blocked_response(self, reason: str, retry_after: float) -> Dict[str, Any]:
        ...
        return {
            ...
            "detected_patterns": [],
            "recommended_actions": [],
            "developmental_safety": {"is_safe": True, "reasons": []},
            "interaction_record": {},
        }
...
# Per-user safety engine cache with TTL-based cleanup
from typing import Dict, Optional
import threading
from datetime import datetime, timedelta

_default_config = TemporalSafetyConfig()
_user_engines: Dict[str, Tuple[AIWorkflowSafetyEngine, datetime]] = {}
_cache_lock = threading.Lock()

def get_ai_workflow_safety_engine(user_id: str,
                                config: Optional[TemporalSafetyConfig] = None,
    ...
    """
    # Clean up expired engines (older than 1 hour)
    _cleanup_expired_engines()

    with _cache_lock:
        # Check if we already have an engine for this user
        if user_id in _user_engines:
            engine, created_at = _user_engines[user_id]
            # Update last access time
            _user_engines[user_id] = (engine, datetime.now())
            return engine

        # Create new engine for this user
        engine = AIWorkflowSafetyEngine(user_id, config or _default_config, user_age)
        _user_engines[user_id] = (engine, datetime.now())
        return engine

def _cleanup_expired_engines(max_age_hours: int = 1):
    """Clean up expired safety engine instances to prevent memory leaks"""
    cutoff_time = datetime.now() - timedelta(hours=max_age_hours)

    with _cache_lock:
        expired_keys = [
            user_id for user_id, (_, created_at) in _user_engines.items()
...
def get_global_ai_workflow_safety_engine(config: Optional[TemporalSafetyConfig] = None,
                                       user_age: Optional[int] = None) -> AIWorkflowSafetyEngine:
