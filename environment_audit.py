...
def analyze_environment_scopes():
    ...
    try:
        ...
        conflicts = {k: list(v) for k, v in name_to_paths.items() if len(v) > 1 and not k.startswith("_")}
        ...
        return {
            ...
            "environment_vars": {
                k: v for k, v in os.environ.items() if any(s in k.lower() for s in ["path", "python", "home", "lib"])
            },
