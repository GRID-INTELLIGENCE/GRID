...
class ProtectionEscalationEngine:
    ...
    def get_endpoint_escalation(self, endpoint: str) -> Optional[Dict[str, Any]]:
...
class DRTMonitor:
    ...
    def monitor_endpoint(
        ...
        similar_attacks = self.attack_database.find_similar_attacks(
    ...
    def _cleanup_old_entries(self, endpoint: str):
        ...
        self.behavioral_history[endpoint] = [
            fp for fp in self.behavioral_history[endpoint]
    ...
    def get_system_drt_status(self) -> Dict[str, Any]:
        ...
        escalated_endpoints = sum(
            1 for endpoint in self.behavioral_history.keys()
            if self.escalation_engine.is_endpoint_escalated(endpoint)
        )
...
def check_drt_violation(endpoint: str, method: str, client_ip: str, user_agent: str = "", headers: Dict = None) -> Dict[str, Any]:
