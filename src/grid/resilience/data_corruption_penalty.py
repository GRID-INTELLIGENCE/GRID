...
class DataCorruptionPenaltyConfig(BaseModel):
    ...
    decay_rate: float = Field(
        default=0.9,
        description="Penalty decay rate per hour (0.9 means 10% decay per hour)",
        ge=0.0,
        le=1.0
    )
    max_penalty: float = Field(
        default=100.0,
        description="Maximum penalty score before severe actions are taken",
        gt=0.0
    )
    critical_threshold: float = Field(
        default=50.0,
        description="Threshold for considering an endpoint as critical",
        gt=0.0
    )
...
class DataCorruptionPenaltyTracker:
    ...
    def __init__(self, config: Optional[DataCorruptionPenaltyConfig] = None):
    ...
    def record_corruption(self, event: DataCorruptionEvent) -> float:
        ...
        base_penalty = (
            self.config.severity_weights.get(event.severity, 1.0) *
            self.config.type_weights.get(event.corruption_type, 1.0)
        )
    ...
    def _update_endpoint_penalty(self, endpoint: str) -> None:
        ...
        if updated_penalties:
            ...
            if total_penalty >= self.config.critical_threshold:
                ...
                logger.critical(
                    "Endpoint %s has reached critical penalty threshold (score: %.2f)",
                    endpoint, total_penalty
                )
                ...
                logger.info(
                    "Endpoint %s is no longer critical (score: %.2f)",
                    endpoint, total_penalty
                )
        else:
    ...
    def get_critical_endpoints(self) -> List[Tuple[str, float]]:
        ...
        return [
            (endpoint, self._endpoint_penalties[endpoint])
            for endpoint in self._critical_endpoints
        ]
    ...
    def _get_recommendation(self, penalty: float, is_critical: bool) -> str:
        ...
        if is_critical:
            return ("IMMEDIATE ACTION REQUIRED: Endpoint is causing critical data corruption. "
                   "Consider disabling the endpoint and performing data recovery.")
        
        if penalty > 20.0:
            return ("WARNING: Endpoint has high penalty score. "
                   "Review recent data corruption events and implement fixes.")
