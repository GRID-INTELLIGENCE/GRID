...
class ContractLoader:
    ...
    def __init__(self, config_path: Optional[Path] = None):
        ...
        if config_path is None:
            config_path = Path(__file__).parent.parent.parent.parent.parent / "config" / "accountability" / "contracts.yaml"
        
        self.config_path = Path(config_path)
        self._contract_cache: Optional[AccountabilityContract] = None
    ...
    def _align_endpoint_data(self, endpoint_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "request_validation" in endpoint_data:
            aligned["request_validation"] = self._align_validation_rules(
                endpoint_data["request_validation"]
            )
        
        if "response_validation" in endpoint_data:
            aligned["response_validation"] = self._align_validation_rules(
                endpoint_data["response_validation"]
            )
    ...
    def _align_validation_rules(self, rules_data: Dict[str, Any]) -> Dict[str, DataValidationRule]:
        ...
        for field_name, rule_data in rules_data.items():
            if isinstance(rule_data, dict):
                ...
                aligned_rules[field_name] = DataValidationRule(
                    field=field_name,
                    type=rule_data,
                    required=True
                )
...
_global_contract_loader: Optional[ContractLoader] = None
