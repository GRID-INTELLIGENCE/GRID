...
class ComprehensiveDRTMiddleware(BaseHTTPMiddleware):
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self._initialized = True
            logger.info(f"DRT middleware initialized with {len(self.attack_vectors)} attack vectors and {len(self.behavioral_history)} signatures")
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if similarity >= self.similarity_threshold and matched_vector:
            logger.warning(
                f"DRT: Behavioral similarity detected ({similarity:.2f}) "
                f"with attack vector for {request.url.path}"
            )
    ...
    def _build_signature(self, request: Request) -> BehavioralSignature:
        header_keys = tuple(
            sorted([k for k in request.headers.keys() if k.lower() not in {
    ...
    def _escalate_endpoint(self, path: str) -> None:
        expires_at = datetime.utcnow() + timedelta(minutes=self.escalation_timeout_minutes)
        self.ESCALATED_ENDPOINTS[path] = expires_at
        
        if self.alert_on_escalation:
            logger.warning(
                f"DRT: Endpoint escalated: {path} "
    ...
    async def _record_violation(self, signature: BehavioralSignature, matched_vector: BehavioralSignature, similarity: float, request: Request, processing_time: float) -> None:
    ...
    def _cleanup_old_entries(self) -> None:
        ...
        self.behavioral_history = [
            b for b in self.behavioral_history
    ...
    async def _periodic_cleanup(self) -> None:
        ...
        while True:
            try:
                ...
                now = datetime.utcnow()
                expired = [
                    path for path, expiry in self.ESCALATED_ENDPOINTS.items()
                    if expiry <= now
                ]
                for path in expired:
