...
class UnifiedDRTMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        # Apply sampling
        if random.random() > self.sampling_rate:  # noqa: S311 non-security random use
            return await call_next(request)
        ...
        monitoring_result = check_drt_violation(
            endpoint=path, method=method, client_ip=client_ip, user_agent=user_agent, headers=dict(request.headers)
        )
    ...
    async def _handle_escalation(self, request: Request, monitoring_result: dict[str, Any]) -> None:
        ...
        if self.enforcement_mode == "monitor":
            ...
            self.escalated_endpoints[path] = datetime.now(UTC) + timedelta(minutes=self.escalation_timeout_minutes)
    ...
    def _log_api_movement(
        self, request: Request, response: Response, duration_ms: float, monitoring_result: dict[str, Any]
    ) -> None:
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "behavioral_history_count": sum(len(history) for history in self.drt_monitor.behavioral_history.values()),
    ...
    def add_attack_vector(
        self, endpoint: str, method: str, client_ip: str, user_agent: str, attack_type: str, severity: str
    ) -> None:
