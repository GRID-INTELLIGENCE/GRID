...
from sqlalchemy import Float, Integer, String, Text, DateTime, JSON, Boolean, ForeignKey, Index
...
class DRTViolationRow(Base):
    ...
    # Relationships
    signature: Mapped["DRTBehavioralSignatureRow"] = relationship("DRTBehavioralSignatureRow", back_populates="violations")
...
class DRTEscalatedEndpointRow(Base):
    ...
    similarity_score: Mapped[float] = mapped_column(Float, default=0.0)
    matched_attack_vector_id: Mapped[str | None] = mapped_column(String(64), ForeignKey("drt_attack_vectors.id"), nullable=True)
