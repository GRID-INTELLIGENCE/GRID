...
class SimpleCache:
    """Tiny TTL cache for filesystem read operations."""

    def __init__(self, ttl_seconds: int = 10, max_entries: int = 256):
...
class ProductionFilesystemMCPServer:
    ...
    def __init__(self):
        ...
        self.cache = SimpleCache(ttl_seconds=int(os.getenv("FILESYSTEM_CACHE_TTL", "10")))
    ...
    async def _list_directory(self, arguments: dict[str, Any]) -> CallToolResult:
        ...
        try:
            ...
            content = "\n".join(items)
            response = CallToolResult(content=[TextContent(text=content, type="text")])
