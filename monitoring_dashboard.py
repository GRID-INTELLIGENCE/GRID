...
class GridMonitoringDashboard:
    ...
    def get_agentic_metrics(self):
        ...
        if success:
            try:
                ...
                print(f"  ✗ Failed to parse experience data")
                return False
        else:
            print(f"  ✗ Could not fetch experience metrics")
            return False
    ...
    def get_rag_metrics(self):
        ...
        if rag_db.exists():
            ...
            print(f"  RAG Database: ✗ NOT FOUND")
    ...
    def get_skills_metrics(self):
        ...
        if success:
            try:
                ...
                print(f"  ✗ Failed to parse skills data")
                return False
        else:
    ...
    def get_case_references_metrics(self):
        ...
        if case_refs.exists():
            ...
            print(f"  Case References: Not created yet")
    ...
    def get_system_health(self):
        ...
        if passed == total:
            ...
            print(f"  Status: ⚠ MOSTLY OPERATIONAL")
        else:
    ...
    def run_dashboard(self):
        ...
        if self.metrics.get('rag_db_files', 0) == 0:
            print(f"  → Build RAG index: .venv\\Scripts\\python.exe -m tools.rag.cli index . --rebuild --curate")
        
        if self.metrics.get('core_skills_available', 0) < 3:
            print(f"  → Verify skills: .venv\\Scripts\\python.exe -m grid skills list")
        
        if self.metrics.get('api_health') != 'running':
            print(f"  → Start API server: .venv\\Scripts\\python.exe -m application.mothership.main")
