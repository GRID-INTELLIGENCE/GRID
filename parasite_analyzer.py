...
class ParasiteAnalyzer:
    ...
    def analyze_function(self, func) -> Dict[str, Any]:
        ...
        try:
            ...
            if (findings['patterns_found'] or findings['whitelist_violation'] or
                findings['code_quality_issue'] or findings['security_risk']):
    ...
    def generate_report(self, output_file: str = 'parasite_analysis_report.json'):
        ...
        self.analysis_results['suspicious_functions'].sort(
            key=lambda x: (
                x['security_risk'],
                x['whitelist_violation'],
                x['code_quality_issue']
            ),
            reverse=True
        )
...
def main():
    ...
    parser.add_argument('--output', '-o', default='parasite_analysis_report.json',
                       help='Output file for the analysis report')
    parser.add_argument('--recursive', '-r', action='store_true',
                       help='Recursively analyze all Python files in a directory')
